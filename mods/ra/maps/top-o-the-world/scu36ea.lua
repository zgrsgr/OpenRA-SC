...
--Objectives Setup
	...
	BringSupplyTruck = player.AddPrimaryObjective("护送运载卡车至山顶的中转站")
	ProtectWaystation = player.AddPrimaryObjective("中转站不得遭到破坏")
	DestroyAAGuns = player.AddSecondaryObjective("摧毁此地所有盟军防空炮以呼叫空中支援")
	PreventAlliedIncursions = player.AddSecondaryObjective("搜寻并切断盟军在此地用于增派支援的桥梁")
	...
	Trigger.OnAllKilled(AlliedAAGuns, function()
		...
		Trigger.AfterDelay(DateTime.Seconds(2), function()
			...
			Media.DisplayMessage("干得好！空中支援已上路，预计3分钟后抵达！")
		end)
  end)
	...
	Trigger.AfterDelay(0, function()
		...
		Trigger.OnKilled(BridgeEnd, function()
			...
			Trigger.AfterDelay(DateTime.Seconds(2), function()
				...
				Media.DisplayMessage("盟军拦截部队已无法再获得增援")
			end)
		end)
	end)
