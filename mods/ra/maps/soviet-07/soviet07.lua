...
WorldLoaded = function()
	...
	sovietObjective1 = player.AddPrimaryObjective("解除安全系统。")
	sovietObjective2 = player.AddPrimaryObjective("营救工程师。")
	sovietObjective3 = player.AddPrimaryObjective("让工程师到达冷却站。")
	sovietObjective4 = player.AddPrimaryObjective("让工程师重新为安全系统编程。")
	sovietObjective5 = player.AddPrimaryObjective("让工程师接近反应堆核心。")
	sovietObjective6 = player.AddSecondaryObjective("释放被困的军犬。")
end
...
Tick = function()
	...
	if remainingTime > 0 and timerStarted then
		UserInterface.SetMissionText("距反应堆自毁还有： " .. Utils.FormatTime(remainingTime), player.Color)
